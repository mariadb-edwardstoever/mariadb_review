...
SET @MUST_SET_TO_ZERO='TRUE'; -- DEFAULT
...
if @MUST_SET_TO_ZERO='TRUE' AND @DO_NOTHING != 'YES' THEN
...
if @SESS_WSREP_ON = 'OFF' and @IS_GALERA = 'YES' and @AUTHORIZED_TO_WSREP_ON='TRUE' THEN
  ...
  select ID into @CURRENT_ITERATION from mariadb_review.ITERATION where 1=1 limit 1;
  if @CURRENT_ITERATION = 0 then
    select concat('Current iteration is 0. Nothing done.') as `NOTE`
    from information_schema.SCHEMATA where SCHEMA_NAME='mariadb_review';
  else
    select concat('Setting Mariadb_review iteration to 0.') as NOTE 
    from information_schema.SCHEMATA where SCHEMA_NAME='mariadb_review';
