...
/* COLLECT_PERF_STATS_PER_MINUTE is the number of times the script will collect performance stats each minute. */
/* It is useful to collect perf stats frequently when trying to trap a specific event that is visible only breifly. */
/* Allowed values for @COLLECT_PERF_STATS_PER_MINUTE are 1, 2, 3, 4, 5, 6, 10, 12, 15, 20, 30, 60 */
/* In most cases, COLLECT_PERF_STATS_PER_MINUTE should be 1 */
set @COLLECT_PERF_STATS_PER_MINUTE=1;
...
/* When the server is primary/master or member of Galera, REPLICATE='NO' will */
/* turn off replication in the session running the script. SUPER or BINLOG ADMIN privilege is required. */
/* The benefit to REPLICATE='NO' is to ensure that data collected on a server remains pertinent to itself. */
/* There is a risk of breaking replication by setting REPLICATE='NO'. Read the included document KNOWN_RISKS.md. */
/* If you are unsure what to do, you should use REPLICATE='YES'. */
/* REPLICATE='NO' will be ignored if user running script does not have privileges or when the server is stand-alone. */
/* VALID VALUES: ['YES'|'NO'] DEFAULT VALUE: 'YES' */
set @REPLICATE='YES';

/* -------- DO NOT MAKE CHANGES BELOW THIS LINE --------- */
set @MARIADB_REVIEW_VERSION='1.8.0';
...
SET @DO_NOTHING='NO'; -- USED TO ESCAPE WHEN A PROBLEM IS DETECTED. 
SET @WINDOWS='NO';

/* Is the host operating system a version of Windows? */
select 'YES' into @WINDOWS from information_schema.GLOBAL_VARIABLES 
where VARIABLE_NAME='version_compile_os' and VARIABLE_VALUE like '%win%';
...
/* Ensuring that @COLLECT_PERF_STATS_PER_MINUTE is a factor of 60 */
set @COLLECT_PERF_STATS_PER_MINUTE=if(@COLLECT_PERF_STATS_PER_MINUTE<1,1,@COLLECT_PERF_STATS_PER_MINUTE);
set @COLLECT_PERF_STATS_PER_MINUTE=if(@COLLECT_PERF_STATS_PER_MINUTE between 7 and 9,6,@COLLECT_PERF_STATS_PER_MINUTE);
set @COLLECT_PERF_STATS_PER_MINUTE=if(@COLLECT_PERF_STATS_PER_MINUTE=11,10,@COLLECT_PERF_STATS_PER_MINUTE);
set @COLLECT_PERF_STATS_PER_MINUTE=if(@COLLECT_PERF_STATS_PER_MINUTE between 13 and 14,12,@COLLECT_PERF_STATS_PER_MINUTE);
set @COLLECT_PERF_STATS_PER_MINUTE=if(@COLLECT_PERF_STATS_PER_MINUTE between 16 and 19,15,@COLLECT_PERF_STATS_PER_MINUTE);
set @COLLECT_PERF_STATS_PER_MINUTE=if(@COLLECT_PERF_STATS_PER_MINUTE between 21 and 29,20,@COLLECT_PERF_STATS_PER_MINUTE);
set @COLLECT_PERF_STATS_PER_MINUTE=if(@COLLECT_PERF_STATS_PER_MINUTE between 31 and 59,30,@COLLECT_PERF_STATS_PER_MINUTE);
set @COLLECT_PERF_STATS_PER_MINUTE=if(@COLLECT_PERF_STATS_PER_MINUTE>60,60,@COLLECT_PERF_STATS_PER_MINUTE);

set @TIMES_TO_COLLECT_PERF_STATS = (@MINUTES_TO_COLLECT_PERF_STATS*@COLLECT_PERF_STATS_PER_MINUTE);
set @MINUTE_DIV=floor(60/@COLLECT_PERF_STATS_PER_MINUTE);
set @NEXT_BY_SECONDS=0;
set @REMAINING=@TIMES_TO_COLLECT_PERF_STATS;
...
select 'UPDATE'  as PRIVILEGE UNION ALL
select 'DELETE'  as PRIVILEGE UNION ALL
...
ON (A.PRIVILEGE=B.PRIVILEGE_TYPE AND replace(B.GRANTEE,'''','')=current_user())
 ...
 IF @MISSING_PRIVS != 'NONE' THEN
  SELECT concat('Insufficient privileges. Grant ',@MISSING_PRIVS,' on *.* to ',CONCAT('\'',REPLACE(CURRENT_USER(),'@','\'@\''),'\''),'. Doing nothing.') as NOTE;
  SET @DO_NOTHING='YES';
 END IF;
end;
//
...
if @WINDOWS='NO' then
    select concat('set session wsrep_on=@SESS_WSREP_ON;') as txt into @SQL1;
...
SET @PREVIOUS_HOST='no_info_available';
...
/* IF THIS IS MASTER, REPLICATE=YES, MARIADB_REVIEW schema DOES NOT EXIST, */
/* DROP the schema TO ENSURE REPLICATION DOES NOT BREAK to a replica that has the schema. */
IF @IS_PRIMARY='YES' AND @REPLICATE='YES' AND @MARIADB_REVIEW_SCHEMA_EXISTS='NO' then
  DROP SCHEMA IF EXISTS mariadb_review; -- THIS WILL DROP ON REPLICA
END IF;
...
if @IS_REPLICA = 'YES' OR @IS_GALERA = 'YES' THEN
  ...
  select concat('Use the clean_up.sql script on the host ',@PREVIOUS_HOST,' before running mariadb_review.sql on this host.') as `NOTE`;
else
  set @DO_NOTHING='YES';
  select concat('Use the clean_up.sql script before running mariadb_review.sql.') as `NOTE`;
END IF;
END IF;

/* YOU CANNOT TURN OFF REPLICATION IF NOT AUTHORIZED BY CORRECT PRIVILEGE */
...
IF @IS_PRIMARY='YES' AND @REPLICATE!='YES' AND @AUTHORIZED_TO_SQL_LOG_BIN='FALSE' THEN
  set @DO_NOTHING='YES';
  select concat('This is a master. REPLICATE=NO and user has insufficient privileges. Doing nothing.') as NOTE;
END IF;

IF @IS_GALERA='YES' AND @REPLICATE!='YES' AND @AUTHORIZED_TO_WSREP_ON='FALSE' THEN
  set @DO_NOTHING='YES';
  select concat('This is a Galera server. REPLICATE=NO and user has insufficient privileges. Doing nothing.') as NOTE;
END IF;
end;
//
delimiter ;
...
/* If pevious run exists and @REPLICATE changes from that run, previous mariadb_review schema must be dropped */
/* to ensure drop is replicated according to previous rules so that this script does not break replication. */
CASE
  WHEN @REPLICATE != @PREVIOUS_REPLICATE THEN
    SET @MUST_DROP='TRUE';
  ELSE
    SET @MUST_DROP='FALSE';
END CASE;

/* OVERRIDE MUST_DROP WHEN DROP_OLD_SCHEMA_CREATE_NEW=YES */
if @DROP_OLD_SCHEMA_CREATE_NEW='YES' and @MUST_DROP='FALSE' then 
  set @MUST_DROP='TRUE'; 
end if;
...
if @SESS_WSREP_ON = 'OFF' and @IS_GALERA = 'YES' and @AUTHORIZED_TO_WSREP_ON='TRUE' THEN
  if @WINDOWS='NO' then
    select concat('SET SESSION WSREP_ON=OFF;') as txt into @SQL1;
    PREPARE STMT1 FROM @SQL1;
    EXECUTE STMT1;
    DEALLOCATE PREPARE STMT1;
  end if;
...
if @REPLICATE != @PREVIOUS_REPLICATE then
  ...
  (SELECT concat('REPLICATE changed. Must drop old schema.') as `NOTE`) as `NOTE`)
...
/* Put session variables for replication ON -- default behavior in most cases. */
 ...
 END IF;
  ...
  if @WINDOWS='NO' then
    select concat('SET SESSION WSREP_ON=ON;') as txt into @SQL1;
    PREPARE STMT1 FROM @SQL1;
    EXECUTE STMT1;
    DEALLOCATE PREPARE STMT1;
  end if;
...
if @REPLICATE !='YES' then
  ...
  if @AUTHORIZED_TO_WSREP_ON='TRUE' THEN
    select concat('SET SESSION WSREP_ON=OFF;') as txt into @SQL1;
    PREPARE STMT1 FROM @SQL1;
    EXECUTE STMT1;
    DEALLOCATE PREPARE STMT1;
...
if @SERVER_STATE_EXISTS='YES' AND @DO_NOTHING != 'YES' THEN
  /* DROP TABLE IF EXISTS COULD PREVENT A PROBLEM WITH REPLICATION */
  select concat('DROP TABLE IF EXISTS mariadb_review.SERVER_STATE_OLD_',date_format(str_to_date(`STATUS`,'%Y-%m-%d %H:%i:%S'),'%Y_%m_%d_%H_%i_%S'))  into @SQL1 
  ...
  select concat('RENAME TABLE mariadb_review.SERVER_STATE TO mariadb_review.SERVER_STATE_OLD_',date_format(str_to_date(`STATUS`,'%Y-%m-%d %H:%i:%S'),'%Y_%m_%d_%H_%i_%S'))  into @SQL2 
  from mariadb_review.SERVER_STATE where ITEM='REVIEW STARTS'
  and exists (select 'x' from mariadb_review.CURRENT_RUN where ID = 1 and STATUS != 'RUNNING')
  LIMIT 1;
  
  if @SQL2 is not null then
    select concat('Renaming table from SERVER_STATE TO SERVER_STATE_OLD_',date_format(str_to_date(`STATUS`,'%Y-%m-%d %H:%i:%S'),'%Y_%m_%d_%H_%i_%S'),'.')  as `NOTE` from mariadb_review.SERVER_STATE where ITEM='REVIEW STARTS' LIMIT 1;  
    PREPARE STMT1 FROM @SQL1;
    EXECUTE STMT1;
...
and not exists (select 'x' from information_schema.SCHEMATA where SCHEMA_NAME='mariadb_review');
...
CREATE TABLE IF NOT EXISTS `CURRENT_RUN` (
  ...
  `RUN_BY` varchar(385) DEFAULT current_user(),
  `RUN_ON` varchar(256) DEFAULT @@hostname,
  ...
  `REPLICATE` varchar(10) DEFAULT NULL,
  `SESSION_SQL_LOG_BIN` varchar(10) DEFAULT NULL,
  `SESSION_WSREP_ON` varchar(10) DEFAULT NULL,
  PRIMARY KEY (`ID`)
...
COMMENT='TO POPULATE PROCESSLIST TABLE SEE THE SCRIPT VARIABLE COLLECT_PROCESSLIST';

drop table if exists `GTID_POSITIONS`;
CREATE TABLE IF NOT EXISTS `GTID_POSITIONS` (
  `ID` int(11) NOT NULL AUTO_INCREMENT,
  `RUN_ID` varchar(12) DEFAULT NULL,
  `TICK` timestamp NOT NULL DEFAULT current_timestamp(),
  `HOSTNAME` varchar(128) DEFAULT NULL,
...
and cast(date_format(TICK,'%S') as integer)=0;
...
COM_DML - (LAG(COM_DML,1) OVER (ORDER BY ID)) as COM_DML_PER_TICK,
...
IBP_READS - (LAG(IBP_READS,1) OVER (ORDER BY ID)) as IBP_READS_PER_TICK,
IBP_READ_REQUESTS - (LAG(IBP_READ_REQUESTS,1) OVER (ORDER BY ID)) as IBP_READ_REQUESTS_PER_TICK,
...
INSERT IGNORE INTO `GALERA_MARIADB_VERSIONS` VALUES 
  ('26.4.18','11.2.4'),('26.4.18','11.1.5'),('26.4.18','11.0.6'),('26.4.18','10.11.8'),
  ('26.4.18','10.6.18'),('26.4.18','10.5.25'),('26.4.18','10.4.34'),
  ('26.4.16','11.2.2'),('26.4.16','11.1.3'),('26.4.16','11.0.4'),('26.4.16','10.11.6'),
  ('26.4.16','10.10.7'),('26.4.16','10.6.16'),('26.4.16','10.5.23'),('26.4.16','10.4.32'),
  ('26.4.17','10.6.17'),('26.4.0','10.4.2'),('26.4.1','10.4.3'),('26.4.11','10.4.22'),
...
if((max(INNODB_BUFFER_POOL_DATA)/1024/1024/1024)<1,
    concat(format(max(INNODB_BUFFER_POOL_DATA)/1024/1024,2),' MB'),
    concat(format(max(INNODB_BUFFER_POOL_DATA)/1024/1024/1024,2),' GB'))as `STATUS`, 
concat('') as `TYPICAL`,
...
if @DO_NOTHING !='YES' THEN
  SELECT SESSION_VALUE into @SESS_SQL_LOG_BIN FROM information_schema.SYSTEM_VARIABLES WHERE VARIABLE_NAME ='SQL_LOG_BIN';
  SELECT SESSION_VALUE into @SESS_WSREP_ON FROM information_schema.SYSTEM_VARIABLES WHERE VARIABLE_NAME ='WSREP_ON';
  INSERT INTO CURRENT_RUN (RUN_ID, `REPLICATE`, SESSION_SQL_LOG_BIN, SESSION_WSREP_ON) values (@RUNID,@REPLICATE,@SESS_SQL_LOG_BIN,@SESS_WSREP_ON)
  ON DUPLICATE KEY UPDATE `RUN_ID`=@RUNID, `RUN_BY`=current_user(), `RUN_ON`=@@hostname,`RUN_START`=now(), 
  RUN_END=NULL, STATUS='RUNNING', STATS_COLLECTED=0,
  `REPLICATE`=@REPLICATE,SESSION_SQL_LOG_BIN=@SESS_SQL_LOG_BIN, SESSION_WSREP_ON=@SESS_WSREP_ON;
  ...
  truncate table GLOBAL_STATUS;
  insert into GLOBAL_STATUS (VARIABLE_NAME,VARIABLE_VALUE) 
  select * from information_schema.GLOBAL_STATUS order by VARIABLE_NAME asc;
...
begin not atomic
if @DO_NOTHING = 'YES' then
 select concat('Doing nothing. You may want to start over by running the clean_up.sql script.') as `NOTE`;
else
...
/* mysql.server_audit_filters may be an ARIA table, in which case, you get an error. */
/* "Transactional commit not supported by involved engine(s)" */
/* So select count(*) into variable. */
  select count(*) into @AUDIT_FILTER_COUNT from mysql.server_audit_filters limit 1;
  insert into `SERVER_STATE` (`SECTION_ID`,`ITEM`,`STATUS`)
  select 1 as `SECTION_ID`, 'ENTERPRISE AUDIT PLUGIN INSTALLED' as `ITEM`,
  concat(format(@AUDIT_FILTER_COUNT,0),if(@AUDIT_FILTER_COUNT=1,' rule',' rules'),' in mysql.server_audit_filters') as `STATUS`
  from information_schema.schemata where SCHEMA_NAME='mariadb_review' limit 1;
end if;
/* Is Audit Plugin installed? */
...
if @IS_GALERA ='YES' THEN
  ...
  insert into `SERVER_STATE` (`SECTION_ID`,`ITEM`,`STATUS`)
  select 2, 'RUNNING GALERA VERSION', substring_index(VARIABLE_VALUE,'-',1) 
  from information_schema.GLOBAL_STATUS where VARIABLE_NAME='WSREP_PROVIDER_VERSION';
  insert into `SERVER_STATE` (`SECTION_ID`,`ITEM`,`STATUS`)
...
if @IS_REPLICA = 'YES' then
  insert into `SERVER_STATE` (`SECTION_ID`,`ITEM`,`STATUS`)
  select 5, VARIABLE_NAME, VARIABLE_VALUE from information_schema.global_variables where VARIABLE_NAME = 'READ_ONLY';
end if;
...
/* Collect a single PROCESSLIST */
if @COLLECT_PROCESSLIST != 'YES' then
  insert into PROCESSLIST (`RUN_ID`,`TICK`,`CONNECTION_ID`,`USER`,`HOST`,`DB`,`COMMAND`,`TIME`,`STATE`,`INFO`,`TIME_MS`,`STAGE`,`MAX_STAGE`,`PROGRESS`,`MEMORY_USED`,`MAX_MEMORY_USED`,`EXAMINED_ROWS`,`QUERY_ID`,`INFO_BINARY`,`TID`)
  select @RUNID, now(), `ID`,`USER`,`HOST`,`DB`,`COMMAND`,`TIME`,`STATE`,`INFO`,`TIME_MS`,`STAGE`,`MAX_STAGE`,`PROGRESS`,`MEMORY_USED`,`MAX_MEMORY_USED`,`EXAMINED_ROWS`,`QUERY_ID`,`INFO_BINARY`,`TID` 
  from information_schema.PROCESSLIST
...
if @MINUTES_TO_COLLECT_PERF_STATS = 1 AND @COLLECT_PERF_STATS_PER_MINUTE then
  SIGNAL SQLSTATE '01000' 
  SET MESSAGE_TEXT="Set MINUTES_TO_COLLECT_PERF_STATS to 2 or more in order to generate comparison values.", 
  MYSQL_ERRNO = 1000;
  show warnings;
end if;

if @MINUTES_TO_COLLECT_PERF_STATS != 0 then
select concat('Performance data collection will begin in ',(60 -cast(date_format(now(),'%S') as integer)),' seconds and will complete in ',format(@MINUTES_TO_COLLECT_PERF_STATS,0), if(@MINUTES_TO_COLLECT_PERF_STATS=1,' minute', ' minutes'),' and ', (60 -cast(date_format(now(),'%S') as integer)),if((60 -cast(date_format(now(),'%S') as integer))=1,' second.',' seconds.')) as NOTE
from information_schema.SCHEMATA where SCHEMA_NAME='mariadb_review'
and cast(date_format(now(),'%S') as integer) !=0
UNION ALL
select concat('Performance data collection will complete in ',format(@MINUTES_TO_COLLECT_PERF_STATS,0), if(@MINUTES_TO_COLLECT_PERF_STATS=1,' minute.', ' minutes.')) as NOTE
from information_schema.SCHEMATA where SCHEMA_NAME='mariadb_review'
and cast(date_format(now(),'%S') as integer) =0;
end if;
...
/* GOING TO START AT @NEXT_BY_SECONDS  */
START_AT_NEXT_BY_SECONDS: WHILE cast(date_format(now(),'%S') as integer) != @NEXT_BY_SECONDS DO
  DO SLEEP(0.1);
END WHILE START_AT_NEXT_BY_SECONDS;

insert into `GTID_POSITIONS` (RUN_ID,TICK,HOSTNAME,GTID_BINLOG_POS,GTID_BINLOG_STATE,GTID_CURRENT_POS,GTID_SLAVE_POS)
WITH GTID_VALUES AS (
SELECT
...
(select VARIABLE_VALUE from information_schema.GLOBAL_VARIABLES where VARIABLE_NAME='GTID_SLAVE_POS' limit 1) AS `GTID_SLAVE_POS`)
 SELECT @RUNID,now(),@@hostname,GTID_BINLOG_POS,GTID_BINLOG_STATE,GTID_CURRENT_POS,GTID_SLAVE_POS from GTID_VALUES;
...
select concat('Performance statistics run number ',format((@TIMES_TO_COLLECT_PERF_STATS - @REMAINING)+1,0),' of ',format(@TIMES_TO_COLLECT_PERF_STATS,0),' completed. (',date_format(now(),'%H:%i:%S'),')') as NOTE;

update CURRENT_RUN set STATS_COLLECTED=STATS_COLLECTED+1 where ID=1;

select ID into @REMAINING from ITERATION where 1=1 limit 1;
 
set @NEXT_BY_SECONDS=@NEXT_BY_SECONDS+@MINUTE_DIV;
if @NEXT_BY_SECONDS >= 60 then set @NEXT_BY_SECONDS=0; end if;
set @PERFORMANCE_SAMPLES = @PERFORMANCE_SAMPLES + 1;

-- important for 1 minute interval when next collection not divided into seconds:
IF @COLLECT_PERF_STATS_PER_MINUTE=1 AND @REMAINING > 0 then do sleep(55); end if; 
