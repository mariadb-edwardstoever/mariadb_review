...
select exists(select 'x' from `WARNINGS` where ITEM='REDO OCCUPANCY IS HIGH') into @OCCUPANCY_WARNING;
  if @OCCUPANCY_WARNING = 0 then
    insert into `WARNINGS` (RUN_ID,HOSTNAME,ITEM,STATUS,INFO) VALUES 
    (@RUNID,@@hostname, 'REDO OCCUPANCY IS HIGH',@OCCUPANCY, NULL);
  ELSE
    update `WARNINGS` set RUN_ID=@RUNID, STATUS=@OCCUPANCY where ITEM='REDO OCCUPANCY IS HIGH' and STATUS < @OCCUPANCY;
  end if;
end if;
...
if ii != @TIMES_TO_COLLECT_PERF_STATS then do sleep(55); end if; 
...
insert into `SERVER_STATE` (`SECTION_ID`,`ITEM`,`STATUS`)  
  select distinct 6, ITEM, STATUS from WARNINGS 
  where RUN_ID = @RUNID;
