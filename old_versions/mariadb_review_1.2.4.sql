...
set @MARIADB_REVIEW_VERSION='1.2.4';
...
CREATE TABLE IF NOT EXISTS `SERVER_PERFORMANCE` (
  ...
  `IBP_READS` int(11) DEFAULT NULL,
  `IBP_READ_REQUESTS` int(11) DEFAULT NULL,
  `MEMORY_USED` int(11) DEFAULT NULL,
  `INNODB_BUFFER_POOL_DATA` int(11) DEFAULT NULL,
  `BINLOG_COMMITS` int(11) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb3 COLLATE=utf8mb3_general_ci;
...
insert into `REVIEW_WARNINGS` (RUN_ID,HOSTNAME,ITEM,STATUS,INFO)
SELECT @RUNID,@@hostname,
substr(concat('LOW CARDIN IDX ON ',A.TABLE_SCHEMA,'.',A.TABLE_NAME),1,100) as ITEM,
...
(RUN_ID,TICK,HOSTNAME,REDO_LOG_OCCUPANCY_PCT,THREADS_CONNECTED,HANDLER_READ_RND_NEXT,COM_SELECT,COM_DML,COM_XA_COMMIT,SLOW_QUERIES,LOCK_CURRENT_WAITS,IBP_READS,IBP_READ_REQUESTS,MEMORY_USED, INNODB_BUFFER_POOL_DATA, BINLOG_COMMITS)
SELECT @RUNID, now(), @@hostname, @OCCUPANCY, @THREADS, @RND_NEXT, @COM_SEL, @COM_DML, @COM_XA, @SLOW_Q, @ROW_LOCK_CURRENT_WAITS, @IBP_READS, @IBP_READ_REQS,@MEM_USED,@BUFFER_POOL_DATA,@BINLOG_TXNS;
...
if @BINLOG_COMMITS_MIN is not null then
  insert into `SERVER_STATE` (`SECTION_ID`,`ITEM`,`STATUS`)
  select 4, 'MAX TXNS WRITTEN TO BINARY LOG 1 MIN',@BINLOG_COMMITS_MIN where @BINLOG_COMMITS_MIN > 0;
end if;

if @TOP_XA_COMMITS_MIN is not null then
  insert into `SERVER_STATE` (`SECTION_ID`,`ITEM`,`STATUS`)
  select 4, 'MAX XA COMMITS IN 1 MIN',@TOP_XA_COMMITS_MIN where @TOP_XA_COMMITS_MIN > 0;
end if;

if @TOP_SLOW_QUERIES is not null then
  insert into `SERVER_STATE` (`SECTION_ID`,`ITEM`,`STATUS`)
  select 4, 'MAX SLOW QUERIES IN 1 MIN',@TOP_SLOW_QUERIES where @TOP_SLOW_QUERIES > 0;
end if;
...
if @TOP_MEMORY_USED is not null then
  insert into `SERVER_STATE` (`SECTION_ID`,`ITEM`,`STATUS`)
  select 4, 
...
if @TOP_BUFFER_POOL_DATA is not null then
  insert into `SERVER_STATE` (`SECTION_ID`,`ITEM`,`STATUS`)
  select 4, 
...
insert into `SERVER_STATE` (`SECTION_ID`,`ITEM`,`STATUS`)  
  select 6, substr(ITEM,1,72), substr(STATUS,1,72) from REVIEW_WARNINGS 
  where RUN_ID = @RUNID;
