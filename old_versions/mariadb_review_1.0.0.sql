...
set @MARIADB_REVIEW_VERSION='1.0.0';
...
select ID, RUN_ID, TICK, HOSTNAME, REDO_LOG_OCCUPANCY_PCT, THREADS_RUNNING, LOCK_CURRENT_WAITS,
...
select A.ID as ID, B.TITLE as SECTION, A.ITEM as ITEM, A.STATUS as STATUS
...
(RUN_ID,TICK,HOSTNAME,REDO_LOG_OCCUPANCY_PCT,THREADS_RUNNING,HANDLER_READ_RND_NEXT,COM_SELECT,COM_DML,LOCK_CURRENT_WAITS)
...
INTO @TOP_REDO_OCPCY, @TOP_THREADS_RUNNING, @TOP_RND_NEXT, @TOP_SELECT_MIN, @TOP_DML_MIN, @TOP_CURRENT_WAITS
...
insert into `SERVER_STATE` (`SECTION_ID`,`ITEM`,`STATUS`)
select 4, 'THREADS RUNNING / MAX CONNECTIONS',concat(@TOP_THREADS_RUNNING,' / ',@@MAX_CONNECTIONS);
