...
set @TIMES_TO_COLLECT_PERF_STATS=5;
...
set @MARIADB_REVIEW_VERSION='1.2.2';
...
/* SECTION 5 GLOBALS */
insert into `SERVER_STATE` (`SECTION_ID`,`ITEM`,`STATUS`)
select 5, concat(VARIABLE_NAME,' MB'), concat(format(VARIABLE_VALUE/1024/1024,0),'M')
...
(RUN_ID,TICK,HOSTNAME,REDO_LOG_OCCUPANCY_PCT,THREADS_CONNECTED,HANDLER_READ_RND_NEXT,COM_SELECT,COM_DML,COM_XA_COMMIT,SLOW_QUERIES,LOCK_CURRENT_WAITS,IBP_READS,IBP_READ_REQUESTS)
SELECT @RUNID, now(), @@hostname, @OCCUPANCY, @THREADS, @RND_NEXT, @COM_SEL, @COM_DML, @COM_XA, @SLOW_Q, @ROW_LOCK_CURRENT_WAITS, @IBP_READS, @IBP_READ_REQS;
...
INTO @TOP_REDO_OCPCY, @TOP_THREADS_CONNECTED, @TOP_RND_NEXT, @TOP_SELECT_MIN, @TOP_DML_MIN, @TOP_XA_COMMITS_MIN, @TOP_SLOW_QUERIES, @TOP_CURRENT_WAITS, @LOW_CACHE_HITS
...
if @LOW_CACHE_HITS is not null THEN
  insert into `SERVER_STATE` (`SECTION_ID`,`ITEM`,`STATUS`)
  select 4, 'LOWEST BUFFER CACHE HIT PCT 1 MIN',format(@LOW_CACHE_HITS,6);
end if;
