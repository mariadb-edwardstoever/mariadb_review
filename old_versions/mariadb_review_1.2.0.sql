...
/* You can set @TIMES_TO_COLLECT_PERF_STATS to a very large number to run nearly indefinitely. */
/* Stop the script gracefully from another session by updating the ID column on ITERATION table: */
...
set @MARIADB_REVIEW_VERSION='1.2.0';
set @REDO_WARNING_THRESHOLD=12;
set @LONG_RUNNING_TRX_THRESHOLD_MINUTES = 1; 
...
if @TIMES_TO_COLLECT_PERF_STATS = 1 then
  ...
  SET MESSAGE_TEXT="You should set TIMES_TO_COLLECT_PERF_STATS to 2 or more in order to generate comparison values.", 
  MYSQL_ERRNO = 1000;
  show warnings;
end if;
...
/*
if @REPLICA_RUNNING = 'YES' then
  insert into `WARNINGS` (RUN_ID,HOSTNAME,ITEM,STATUS,INFO)
  ...
  and `STATE` like 'Update_rows_log_event::find_row(-1) on table%';
  ...
  where `USER` = 'system user' 
  and `COMMAND` in ('Slave_SQL','Slave_worker')
  and `STATE` like 'Delete_rows_log_event::find_row(-1) on table%';
...
-- if (abs(TIMESTAMPDIFF(HOUR,now(), A.trx_started)) < 2,
-- concat(B.USER,'@',B.HOST,', trx started ',format(abs(TIMESTAMPDIFF(MINUTE, A.trx_started,now())),0),' minutes ago'),
-- concat(B.USER,'@',B.HOST,', trx started ',format(abs(TIMESTAMPDIFF(HOUR, A.trx_started,now())),0),' hours ago')
-- ) as STATUS,
...
insert into `SERVER_STATE` (`SECTION_ID`,`ITEM`,`STATUS`)  
  select distinct 6, ITEM, STATUS from REVIEW_WARNINGS 
  where RUN_ID = @RUNID;
